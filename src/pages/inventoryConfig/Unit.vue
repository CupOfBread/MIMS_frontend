...
import AddUnit from '@/pages/inventoryConfig/AddUnit'
const columns = [
  {
    title: '计量单位名称',
    dataIndex: 'name'
...
export default {
  name: 'QueryList',
  components: { StandardTable, AddUnit },
  data () {
    return {
      advanced: true,
      columns: columns,
  ...
  mounted () {
    ...
    let that = this
    request('/api/unit/page', 'GET', params, {
      headers: {
        'Authorization': '123'
      }
    }).then(function (res) {
  ...
  methods: {
    ...
    handleOk () {
      let data = this.$refs.addUnit.getVal()
